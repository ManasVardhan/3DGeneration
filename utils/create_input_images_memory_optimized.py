...
def render_view_with_texture(mesh, rotation_matrix, resolution=(512, 512)):
    ...
    try:
        ...
        # CRITICAL: Clean up immediately after rendering
        image.close()
        del scene
        del mesh_copy
...
def process_folder(folder_num):
    ...
    mesh = None  # Initialize mesh variable for cleanup
    ...
    try:
        ...
        for view_name, rotation in views.items():
            ...
            if img is not None:
                ...
                # CRITICAL: Close and delete PIL image immediately
                pil_img.close()
                del pil_img
                del img
        ...
        # CRITICAL: Always clean up mesh and force garbage collection
        if mesh is not None:
            del mesh
        gc.collect()
...
for folder_num in range(1, 106):  # Folders 1 to 105
    ...
    # CRITICAL: Force garbage collection every 5 folders
    if folder_num % 5 == 0:
        gc.collect()
        print(f"\n🧹 Memory cleanup performed (processed {folder_num}/105 folders)")
